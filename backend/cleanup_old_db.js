...
const cleanup = async () => {
    try {
        // Connect to the OLD database specifically to clear it
        await mongoose.connect('mongodb://127.0.0.1:27017/doctor_appointment');
        console.log('Connected to OLD DB (doctor_appointment)...');
        ...
        for (const name of namesToDelete) {
            ...
            for (const user of users) {
                ...
                console.log(`Deleted from Old DB: ${user.name} (${user.email})`);
            }
        }

        console.log('Cleanup of OLD DB completed!');
